...
import {courseExamOptions,courseQuizOptions} from '../courses/course-session.js';
...
function renderBlock(block,ctx) {
  ...
  if(block.type==='definition') {
    ...
    return definition?`<aside class="course-definition"><p class="eyebrow">DÉFINITION</p><h3>${e(definition.terme)}</h3><p>${e(definition.definitionCourte)}</p><a href="#/notions/${e(definition.id)}">Voir la fiche <i class="ph ph-arrow-right" aria-hidden="true"></i></a></aside>`:'';
  }
...
export function coursesScreen(root,ctx) {
  ...
  const render=()=>{
    ...
    root.querySelector('#course-subjects').innerHTML=subjects.map(subject=>{
      ...
      return matches.length?`<a class="card course-subject-card" href="${subjectLink(subject)}"><span><strong>${e(subject)}</strong><small>${matches.length} cours</small></span><i class="ph ph-arrow-right" aria-hidden="true"></i></a>`:'';
...
export function courseSubjectScreen(root,ctx,subject) {
  ...
  root.innerHTML=`<div class="stack"><a class="back-link" href="#/courses"><i class="ph ph-arrow-left" aria-hidden="true"></i> Tous les cours</a><div><p class="eyebrow">MATIÈRE</p><h1 class="mt">${e(subject)}</h1></div><div class="course-list">${courses.map(course=>{
    const progress=courseProgress(course,ctx.user);
    return `<a class="card course-row" href="#/course/${e(course.id)}"><span class="course-order">${String(course.ordre).padStart(2,'0')}</span><span><strong>${e(course.titre)}</strong><small>${course.notions.length} notion${course.notions.length>1?'s':''} · ${progress.coverage}% couvert</small></span><i class="ph ph-arrow-right" aria-hidden="true"></i></a>`;
  }).join('')}</div></div>`;
}
...
export function courseReader(root,ctx,courseId) {
  ...
  root.innerHTML=`<div class="stack"><a class="back-link" href="${subjectLink(course.matiere)}"><i class="ph ph-arrow-left" aria-hidden="true"></i> ${e(course.matiere)}</a><article class="course-article"><header><p class="eyebrow">COURS ${String(course.ordre).padStart(2,'0')}</p><h1>${e(course.titre)}</h1><p class="muted">${e(course.subtitle||course.matiere)}</p><div class="course-progress"><span>${progress.notionsSeen} / ${progress.notionsTotal} notions travaillées</span><strong>Maîtrise : ${progress.mastery} %</strong></div>${bar(progress.coverage,'Couverture du cours '+course.titre)}</header>${toc}${course.sections.map(section=>`<section id="section-${e(section.id)}"><h2>${e(section.titre)}</h2><div class="course-content">${section.contenu.map(block=>renderBlock(block,ctx)).join('')}</div></section>`).join('')}<section class="course-notions"><p class="eyebrow">NOTIONS IMPORTANTES</p><h2>À connaître dans ce cours</h2><div class="course-notion-links">${definitions.map(definition=>`<a href="#/notions/${e(definition.id)}"><strong>${e(definition.terme)}</strong><span>${e(definition.definitionCourte)}</span></a>`).join('')}</div></section><div class="stack"><button id="revise-course" class="button primary wide">RÉVISER CE COURS</button>${course.examCount?`<button id="exam-course" class="button wide">EXAMEN — ${course.examCount} QUESTIONS</button>`:''}</div></article></div>`;
  ...
  const exam=root.querySelector('#exam-course');if(exam)exam.onclick=()=>ctx.start(courseExamOptions(course));
}
