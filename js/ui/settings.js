...
export function settings(root,ctx) {
  root.innerHTML=`<div class="stack">
    ...
    <section class="card stack"><h2>🌿 GPN Quiz</h2>
      ...
      <p class="muted small">Version 1.2 · ${ctx.bank.length} notions officielles · ${ctx.courseIndex.getAllCourses().length} cours<br>Identité de classement anonyme, sans email ni mot de passe.</p>
  ...
  root.querySelector('#display-name').value=ctx.user.profile.displayName;
  root.querySelector('#display-name-form').onsubmit=event=>{event.preventDefault();const status=root.querySelector('#display-name-status'),result=ctx.updateDisplayName(root.querySelector('#display-name').value);status.className='small '+(result.valid?'success':'error');status.textContent=result.valid?'Pseudo enregistré. La synchronisation se fera automatiquement.':result.error;if(result.valid)root.querySelector('#display-name').value=result.displayName;};
  ...
  root.querySelector('#confirm-reset').onclick=()=>{const fresh=defaults();fresh.profile=ctx.user.profile;ctx.replaceUser(fresh);settings(root,ctx);root.querySelector('#import-status').textContent='Progression réinitialisée.';};
