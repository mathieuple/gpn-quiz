...
test('migration V1 conserve scores, compteurs, réglages et original octet pour octet',()=>{
  ...
  assert.equal(user.schemaRevision,2);assert.equal(user.xp,1240);assert.equal(user.bestCombo,12);assert.equal(user.survivalRecord,27);assert.deepEqual(user.settings,legacy.settings);
